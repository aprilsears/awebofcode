<template>
  ...
  <div class="sticky top-0 z-50">
    <header class="bg-steel/15 backdrop-blur-md border-b border-silver/20 px-4 sm:pl-3 sm:pr-8 lg:pl-4 lg:pr-12 py-2 flex flex-row items-center justify-between gap-3">
      ...
      <!-- Desktop nav -->
      <nav aria-label="Main navigation" class="hidden md:flex items-center gap-6 lg:gap-10 text-xs font-semibold tracking-widest uppercase">
        <RouterLink
          v-for="link in links"
          :key="link.to"
          :to="link.to"
      ...
      >
        <svg v-if="!menuOpen" aria-hidden="true" focusable="false" class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16" />
        </svg>
        <svg v-else aria-hidden="true" focusable="false" class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
